...
- **Grok CLI provider (beta)** -- New `grok` provider that signs in with your Grok subscription (SuperGrok / X Premium+) via xAI OAuth 2.0 device flow and proxies the two models served by the Grok CLI endpoint (`cli-chat-proxy.grok.com`) -- **Composer 2.5** (`grok-composer-2.5-fast`) and **Grok Build** (`grok-build`) -- which the public `api.x.ai` API does not expose. Adds a "Log in with Grok" device-code flow in Settings, a provider enable/disable toggle, automatic access-token refresh, and the Factory custom models `custom:droidproxy:grok-composer-2.5-fast` and `custom:droidproxy:grok-build`. Forwards a stable per-conversation `x-grok-conv-id` so cli-chat-proxy's prompt cache hits across turns (verified ~100% cached prompt tokens; without it the full prompt is re-billed every turn). Beta-gated -- enable Beta mode in Settings to use it.
