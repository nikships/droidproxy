...
| `src/Sources/OAuthUsageViews.swift` | Compact quota dashboard: a wrapping flow of ring gauges with no per-account boxes (provider logo inside each ring); two-limit subscriptions render one concentric dual gauge (shorter window inside, longer outside), 3+ stack half-size. An email header only appears above a provider's rings when it has several accounts. Depends only on the usage model types, so it can be rendered outside the full app. |
