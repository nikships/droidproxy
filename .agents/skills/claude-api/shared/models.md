...
### Model Descriptions
- **Claude Opus 4.8** — The most capable Claude model to date — highly autonomous, strong on long-horizon agentic work, knowledge work, vision, and memory. Adaptive thinking only; sampling parameters and `budget_tokens` are removed. 1M context window at standard API pricing (no long-context premium) — see `shared/model-migration.md` → Migrating to Opus 4.8 for breaking changes.
- **Claude Opus 4.8** — Previous-generation Opus. Supports adaptive thinking (recommended), 128K max output tokens (requires streaming for large outputs). 1M context window.
...
|-------------------|---------------------|-------------------------------|--------|
| Claude Opus 4.8   | `claude-opus-4-8`   | `claude-opus-4-8`    | Active |
