...
### Changed
- **Quota rings show provider logos** -- Each quota ring now carries the provider logo inside it (aspect-fit, brand-tinted), and the account email header only appears when a provider has multiple accounts. The exact percent and reset time moved to an instant hover popover, replacing the sluggish system tooltip.
- **Dual-limit concentric rings** -- Every subscription renders the same dual-ring gauge: the shorter window keeps the normal ring with the logo and the longer window wraps it in an outer ring (an empty track when there is no second limit), so all gauges share one size and placement; the hover popover lists each real limit. Three or more windows (only Claude's per-model buckets) stack half-size so no limit is dropped. Window titles were removed from the cards entirely — titles, percents, and reset times all live in the instant hover popover.
- **Compact quota cards** -- **OAuth Quota Usage** is a wrapping flow of ring gauges with no per-account boxes. Each quota window is a hollow ring gauge that drains as quota is used (green, orange below 25%, red below 10%); hover a ring for the window title, exact percent, and reset time.
- **Codex quota window labels** -- Codex windows are labeled from their reported duration, so weekly-only and monthly-only plans show `Weekly` or `Monthly` instead of a mislabeled `5-hour` ring.
...
### Added
- **Meta Muse quota usage** -- The **OAuth Quota Usage** section in Settings shows a card for the Meta account serving Responses traffic, with the 5-hour window and weekly rings plus an "as of" timestamp. Meta exposes no usage endpoint, so ThinkingProxy sniffs the `response.subscription_usage` SSE event from relayed `/v1/responses` streams (bytes pass through unchanged) into `~/.droidproxy/meta/usage.json`, and cards update live as new responses arrive. Accounts that have not served a Responses request yet show a waiting state.
- **SuperGrok quota usage** -- The **OAuth Quota Usage** section in Settings shows a card for the connected Grok account with the pooled weekly SuperGrok credit window, read from `cli-chat-proxy.grok.com/v1/billing?format=credits`. The card follows the Grok provider toggle and the account's disabled state, the same as Codex and Claude.
- **Grok 4.7 Fast** -- Factory custom model `custom:droidproxy:grok-4.7-build-fast` (`grok-4.7-build-fast`, 500k context, default reasoning `xhigh`). Same SuperGrok login as Grok 4.7. The public API does not serve this id, so ThinkingProxy sends it to `cli-chat-proxy.grok.com` with `x-grok-model-override: grok-4.7-build-fast` and `x-grok-client-version` (the proxy rejects a missing version). Re-apply Factory models to register it.
...
- **Cursor Agent CLI proxy (Beta)** -- Replaces the hosted `api-for-cursor.standardagents.ai` API-key path. Composer 2.5 and Grok 4.6 are served from the local Cursor `agent` CLI via `cursor-api-proxy` on `127.0.0.1:8320`. Fast Mode is a Settings toggle (`-fast` suffix). Grok thinking levels are Droid's `reasoning_effort` (`low` / `medium` / `high` / `xhigh`). Composer 2.5 has no thinking variants.
...
- **GPT 5.6 Sol** -- OpenAI's dynamic-reasoning GPT-5.6 tier (`gpt-5.6-sol`) is registered as Factory custom model `custom:droidproxy:gpt-5.6-sol` with the GPT-5.6 Sol effort levels (`dynamic` / `low` / `medium` / `high` / `xhigh` / `max`, default `medium`) and a 128k output-token ceiling. A GPT 5.6 Sol toggle is added to the Codex Fast Mode subsection (`service_tier=priority`). Luna is not registered: `gpt-5.6-luna` is not yet recognized by the bundled CLIProxyAPI backend.
- **GPT 5.6 Terra** -- OpenAI's balanced GPT-5.6 tier (`gpt-5.6-terra`) is registered as Factory custom model `custom:droidproxy:gpt-5.6-terra` with the GPT-5.6 native effort levels (`none` / `low` / `medium` / `high` / `xhigh` / `max`, default `medium`) and a 128k output-token ceiling. A GPT 5.6 Terra toggle is added to the Codex Fast Mode subsection (`service_tier=priority`).
- **Grok 4.6 OAuth** -- SuperGrok / X Premium+ device-flow login against `auth.x.ai`, credentials at `~/.cli-proxy-api/grok-cli.json`, and TLS forwarding to `api.x.ai` for `grok-4.6`. Connect from Settings → Grok, Apply Factory models, then select `/model` → DroidProxy: Grok 4.6. Some SuperGrok tiers may still require Factory BYOK (`XAI_API_KEY`).
- **Claude Opus 5.5** -- Preemptive Anthropic OAuth registration for `claude-opus-5-5` as Factory custom model `custom:droidproxy:opus-5-5`, using the same adaptive-thinking config as Opus 5 (`low` / `medium` / `high` / `xhigh` / `max`, default `xhigh`) and a 128k output-token ceiling.
- **Claude Fable 5.1** -- Anthropic's newest flagship (`claude-fable-5-1`) is registered as Factory custom model `custom:droidproxy:fable-5-1` with the same effort levels as Opus 4.8 (`low` / `medium` / `high` / `xhigh` / `max`, default `xhigh`) and a 128k output-token ceiling.
...
- **Sequential account failover** -- New opt-in checkbox under Settings → Account Routing, for users who stack several accounts on one provider (e.g. multiple Google/Antigravity logins). With it on, DroidProxy stays on a single account until its quota runs out and then moves to the next one automatically, mid-request, without an error reaching Factory/Droid. The exhausted account is parked and skipped until its quota resets, and rejoins the rotation on its own afterwards. Off by default, so existing behavior is unchanged.

  The checkbox rewrites four values in the bundled `config.yaml` and CLIProxyAPI hot-reloads all of them, so no restart is needed: `routing.strategy` `round-robin` → `fill-first`, `disable-cooling` `true` → `false` (so a depleted account is actually parked instead of re-probed on every request), `transient-error-cooldown-seconds` `0` → `-1` (so transient 5xx still never park an auth and the [#57](https://github.com/anand-92/droidproxy/issues/57) blackout stays fixed), and `max-retry-interval` `0` → `30` (wait briefly for the soonest account to recover rather than failing a request outright). Session affinity stays on in both states, so stateful Codex Responses traffic ([#58](https://github.com/anand-92/droidproxy/issues/58)) is unaffected.

  Measured against the bundled CLIProxyAPI backend with five simulated accounts: with the setting on, a depleted account takes **1** upstream call across 10 requests instead of **10**. Note that enabling it also restores the standard 30-minute park on `401`/`403` and 12-hour park on `404` for *all* providers, which is why it is opt-in rather than the default.

### Removed
- **Cursor provider** -- Composer 2.5, Cursor Grok 4.6, the local `cursor-api-proxy` sidecar, Cursor Fast Mode, and the Grok 4.6 Fast Mode divert are gone. Re-apply Factory models to drop `custom:droidproxy:cursor-composer-2.5` and `custom:droidproxy:cursor-grok-4.6`.
- **Cursor Grok 4.5, Cursor Small, and `cursor-grok-4.6-fast` catalog entries** -- No longer registered. Apply/Re-apply prunes leftover Factory ids. Fast Mode is a Settings toggle on `cursor-grok-4.6`, not a separate model.
- **Older Gemini Flash models removed** -- `gemini-3.6-flash-high`, `gemini-3.7-flash-high`, and `gemini-3.1-flash-lite` are no longer registered in the DroidProxy catalog. Gemini 3.8 Flash (High) is the only Flash tier that remains. Re-applying Factory models will prune the old `custom:droidproxy:gemini-3.6-flash-high`, `custom:droidproxy:gemini-3.7-flash-high`, and `custom:droidproxy:gemini-3.1-flash-lite` entries from `~/.factory/settings.json`.
...
### Fixed
- **Muse Spark `/v1/responses` was rewritten into Chat Completions** -- CLIProxyAPI's `openai-compatibility` executor always POSTs `/chat/completions` for Meta, so a Foundry/Muse Responses request came back as `chatcmpl-*` and dropped `reasoning.encrypted_content`. Long Muse agent runs could not persist thinking. ThinkingProxy now TLS-forwards `muse-spark-1.3` / `muse-spark-1.3-contributor` Responses (including `/responses/compact`) straight to `api.meta.ai` with the minted Model API key. Chat Completions still use the compatibility block so multi-account failover is unchanged for Completions clients.

- **Cursor models never called Droid tools** -- `cursor-api-proxy` was pinned to Ask + chat-only so Cursor CLI would not execute its own Write/Shell tools, but Ask mode's personality answered "Ask mode is on" instead of emitting tool calls, and `agent --print` still wrote absolute paths. ThinkingProxy now prepends `CursorClientToolBridge` (and pins `mode=ask`), the sidecar runs `agent` through a `--sandbox enabled` wrapper, and Factory markup or JSON is lifted into OpenAI `tool_calls`. Cursor `Write`/`Delete` names are remapped to Droid `Create` / `Execute rm` so Factory actually runs them.
- **Copilot gateway stuck on "Local gateway starting"** -- The gateway row reported progress from `Process.run()` alone, which only means `npx` was spawned, so a gateway that exited before binding port 8319 (for example when node is missing from the child's minimal PATH and `npx` fails its `#!/usr/bin/env node` shebang with exit 127) displayed "Local gateway starting" indefinitely. `CopilotGatewayManager` now tracks an explicit `idle`/`starting`/`running`/`failed` state and only reports `running` once the gateway answers a `/v1/models` readiness probe. Settings shows the failure reason captured from the gateway's own output next to a Retry button.
- **Grok OAuth tool 422 on `custom`** -- Factory/Droid agent requests often declare tools with `"type":"custom"`, which `api.x.ai` rejects (`unknown variant custom`). The Grok forward path now remaps those entries to `"type":"function"` (and drops types outside xAI's allowlist) before upstream.
...
### Changed
- **Grok 4.7** -- The Factory SuperGrok custom model is now `custom:droidproxy:grok-4.7` (`grok-4.7`, 500k context). Re-applying Factory models replaces `custom:droidproxy:grok-4.6`. Cursor Grok 4.6 is unchanged.
- **Grok 4.6** -- The Factory custom model is now `custom:droidproxy:grok-4.6` (`grok-4.6`, 500k context). Re-applying Factory models replaces `custom:droidproxy:grok-4.5`. Cursor beta is `cursor-grok-4.6` only.
- **Grok Imagine Image 2.0** -- The bundled `skills/grok-imagine` skill now defaults to `grok-imagine-image-2.0`. Quality is `low` / `medium` (default `medium`, 2.0-only; `high` is no longer valid). Multi-image edits are capped at 3 references (`<IMAGE_0>`…`<IMAGE_2>`). Prompting follows the xAI Imagine docs: natural-prose scene description, no negative prompts, edit prompts describe only the change.
