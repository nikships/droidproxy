...
- **Dual-limit concentric rings** -- A subscription with two limits (e.g. 5-hour + weekly) renders one gauge: the shorter window keeps the normal ring with the logo and the longer window wraps it in an outer ring; the hover popover lists both limits. Three or more windows (only Claude's per-model buckets) stack half-size so no limit is dropped. Window titles were removed from the cards entirely — titles, percents, and reset times all live in the instant hover popover; rows bottom-align so ring rows line up across headed and headerless groups.
