...
- **Stacked multi-limit rings** -- A subscription with more than one limit (e.g. 5-hour + weekly) stacks half-size rings vertically into a single spot, shorter window on top. Window titles were removed from the cards entirely — titles, percents, and reset times all live in the instant hover popover; rows bottom-align so ring rows line up across headed and headerless groups.
