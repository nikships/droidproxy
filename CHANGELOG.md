...
### Added
- **Grok 4.7 Fast** -- Factory custom model `custom:droidproxy:grok-4.7-build-fast` (`grok-4.7-build-fast`, 500k context, default reasoning `xhigh`). Same SuperGrok login as Grok 4.7. The public API does not serve this id, so ThinkingProxy sends it to `cli-chat-proxy.grok.com` with `x-grok-model-override: grok-4.7-build-fast` and `x-grok-client-version` (the proxy rejects a missing version). Re-apply Factory models to register it. Cursor models are unchanged.
