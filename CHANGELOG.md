...
- **Stacked multi-limit rings** -- A subscription with more than one limit (e.g. 5-hour + weekly) stacks half-size rings vertically into a single spot, shorter window on top with the titles captioned below; rows bottom-align so ring rows line up across headed and headerless groups.
- **Compact quota cards** -- **OAuth Quota Usage** is a wrapping flow of ring gauges with no per-account boxes. Each quota window is a hollow ring gauge that drains as quota is used (green, orange below 25%, red below 10%); hover a ring for the exact percent and reset time.
