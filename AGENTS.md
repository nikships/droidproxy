...
| `src/Sources/OAuthUsageViews.swift` | Compact quota dashboard: a wrapping flow of ring gauges with no per-account boxes (one hollow draining gauge per window, provider logo inside); multi-window subscriptions stack half-size rings into one spot, shorter window on top. An email header only appears above a provider's rings when it has several accounts. Depends only on the usage model types, so it can be rendered outside the full app. |
