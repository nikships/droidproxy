...
Beta Cursor models instead use:
...
- **Grok 4.6 Fast Mode**: when `AppPreferences.grok46FastMode` is on, `grok-4.6` is rewritten to `cursor-grok-4.6-fast` and diverted to the local Cursor Agent CLI proxy (`cursor-api-proxy` on `127.0.0.1:8320`). Requires Beta → Cursor and `agent login`. `api.x.ai` does not expose a fast variant.
- **Cursor Agent CLI proxy**: beta Cursor models (`cursor-composer-2.5`, `cursor-grok-4.6`) are forwarded to a localhost `cursor-api-proxy` sidecar that wraps the installed `agent` / `cursor-agent` CLI. Fast Mode (`AppPreferences.cursorFastMode`) appends `-fast`. Grok thinking levels are Droid's `reasoning_effort` (`low` / `medium` / `high` / `xhigh`); Composer 2.5 has no thinking variants, so those fields are stripped. The sidecar stays in Ask + chat-only and runs `agent` through `CursorAgentSandboxWrapper` (`--sandbox enabled`) so Cursor cannot execute Write/Shell (Ask mode alone still writes absolute paths). ThinkingProxy injects `CursorClientToolBridge` and lifts Factory markup / JSON into OpenAI `tool_calls` so **Droid** executes Create/Edit/Execute.
- **Grok native tool-call rewrite**: Grok (including `cursor-grok-*`) often leaks Factory markup (`<|tool_calls_begin|>` / `<|tool_call_begin|>`) into `message.content`. Composer may emit a fenced `{"name","arguments"}` JSON object instead. Factory's first-party Grok adapter parses the markup; `generic-chat-completion-api` does not, so the worker turn ends as plain text. The proxy buffers Cursor/Grok responses and lifts markup **or** JSON into OpenAI `tool_calls` before the client sees it.
...
| `src/Sources/ThinkingProxy.swift` | Raw TCP HTTP proxy that forwards requests to CLIProxyAPI (and TLS-forwards Junie/Grok/Meta Responses). Rewrites the Anthropic-Beta header to drop `redact-thinking-2026-02-12` on Claude thinking requests and always strips `fast-mode-*` on Claude traffic, injects `service_tier=priority` on enabled Codex fast-mode models, rewrites OAuth Code Assist Gemini `/v1/responses` to `/v1/chat/completions`, sanitizes Grok tool types before `api.x.ai`, diverts `grok-4.6` to the Cursor Agent CLI proxy as `cursor-grok-4.6-fast` when Grok Fast Mode is on, forwards `cursor-*` models to `cursor-api-proxy` on `127.0.0.1:8320`, prepends `CursorClientToolBridge` so Cursor Ask mode emits Factory tool markup instead of refusing, rewrites that markup (and fenced JSON function calls) into OpenAI `tool_calls` for Droid to execute, repairs Grok `EndFeatureRun`/`Execute` arguments, TLS-forwards Muse Spark `/v1/responses` to `api.meta.ai` so CLIProxyAPI cannot convert them to Completions, and emits a `REQUEST REASONING` log line per request. Does not inject reasoning or thinking fields. |
...
| `src/Sources/CursorAgentProxyManager.swift` | Spawns `npx cursor-api-proxy@1.3.0` on `127.0.0.1:8320`, probes `/healthz`, and reads `agent status --format json` for CLI login. Finder-launched PATH includes `~/.local/bin` and mise/nvm node so `agent` and `npx` resolve. Sidecar uses `agent --print` (ACP off), **ask + chat-only**, and `CURSOR_AGENT_BIN` pointed at `CursorAgentSandboxWrapper` (`--sandbox enabled`) so Cursor cannot execute Write/Shell. `CURSOR_CONFIG_DIRS=~/.cursor` so `agent login` still works. Droid is the agent; ThinkingProxy lifts model markup into OpenAI `tool_calls`. |
| `src/Sources/CursorAgentSandboxWrapper.swift` | Installs `~/Library/Application Support/DroidProxy/cursor-agent-readonly`, which execs the real `agent` with `--sandbox enabled` (login/status bypass the sandbox). |
| `src/Sources/CursorClientToolBridge.swift` | System prompt + `mode=ask` injection on Cursor forwards. Stops the CLI's "Ask mode is on" refusal so Grok/Composer emit Factory tool markup (or JSON) for Droid. |
| `src/Sources/CursorModelRewriter.swift` | Maps `cursor-composer-2.5` → `composer-2.5` and Fast Mode `-fast` suffixes for the Agent CLI catalog. |
...
| `src/Sources/SettingsView.swift` | SwiftUI settings UI in the Factory dark-product theme (`Theme.swift` tokens, numbered section eyebrows, uppercase mono labels): server status, launch-at-login, provider toggles with per-provider switch tints, auth flows, the Codex fast-mode (`service_tier=priority`) subsection, the Grok 4.6 Fast Mode toggle, the Copilot gateway status row (renders the `failed` reason plus a Retry button), the Factory custom-models Apply button, and the collapsed Advanced section (remote access, secret key, verbose logging, logs folder, account failover). No thinking/reasoning selectors — those live in Droid CLI. |
...
| `src/Sources/AppPreferences.swift` | UserDefaults-backed preferences: fast-mode toggles for GPT 5.6-terra/5.6-sol/5.6-luna, Grok 4.6, and Cursor; `allowRemote`, `secretKey`, `verboseLogging`. No thinking-effort keys — reasoning is driven entirely by Droid CLI. |
...
| `src/Sources/LogoView.swift` | Inline-SVG `LogoView` (Factory wordmark + pinwheel) and `DroidGlyph` used in the settings UI. |
| `src/Sources/Theme.swift` | Factory design tokens (#020202 surfaces, hairline borders, orange accent, near-square corners), `PrimaryButtonStyle`/`GhostButtonStyle`, `SectionLabel` numbered eyebrows, and `SurfaceStyle` used across the settings UI. |
