...
- **Claude Opus 5.2** -- Preemptive Anthropic OAuth registration for `claude-opus-5-2` as Factory custom model `custom:droidproxy:opus-5-2`, using the same adaptive-thinking config as Opus 5 (`low` / `medium` / `high` / `xhigh` / `max`, default `xhigh`) and a 128k output-token ceiling.
