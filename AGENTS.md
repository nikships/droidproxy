...
| `src/Sources/OAuthUsageViews.swift` | Compact quota dashboard: a wrapping flow of uniform dual-ring gauges with no per-account boxes (provider logo inside each ring; outer track stays empty without a second limit), 3+ windows stack half-size. An email header only appears above a provider's rings when it has several accounts. Depends only on the usage model types, so it can be rendered outside the full app. |
